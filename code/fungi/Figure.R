...
tiff("F:/CAF/2022-10-20/fungi/fig1/Fig1-2.tiff",width=20,height=13,units = "cm",res=300)
par(mfrow=c(5,6))
...
for (i in 1:5) {
  ...
  for (t in 1:6) {
    ...
    plot(xy[[t]]$X,xy[[t]][,8:l][,example_index],pch=16,cex=1,col=col1[t],
         type = "p",ylab = "",xlab = "",axes=FALSE,ylim = c(-0.1,1.1),
         xlim = c(range(xy[[t]]$X)[1]-0.02,range(xy[[t]]$X)[2]+0.02))
    ...
    if(t==1&&i!=5){
      ...
      axis(1,at=round(seq(range(xy[[t]]$X)[1],range(xy[[t]]$X)[2],length=4),2),labels =FALSE,cex.axis=1)
    }else if(t!=1&&i==5){
      cat("################",round(seq(range(xy[[t]]$X)[1],range(xy[[t]]$X)[2],length=4),2),"\n")
      axis(1,at=round(seq(range(xy[[t]]$X)[1],range(xy[[t]]$X)[2],length=4),2),labels =FALSE,cex.axis=1)
    }
...
tiff("E:/CAF/2022-10-20/fungi/fig1/Fig1-Residuals.tiff",width=20,height=15,units = "cm",res=300)
par(mfrow=c(5,6))
...
k <- 11
# filename<-paste("E:/Doctoral/CAF/2022-10-1/Bacteria/cluster-result/k",k,".RData",sep = "")
# load(filename)
...
tiff("E:/CAF/2022-10-20/fungi/fig2/Fig2-k11.tiff",width=20,height=27,units = "cm",res=300)
par(mfrow=c(k,6))
par(mar = c(0, 0, 0,0),oma = c(1, 1, 1, 1),mgp=c(1,0.5,0))
...
for (i in 1:k) {
  ...
  for (t in 1:6) {
    ...
    plot(xy[[t]]$X,curve,type = "n",ylab = "",xlab = "",axes=FALSE,ylim = c(-0.02,1.1),
         xlim = c(range(xy[[t]]$X)[1]-0.01,range(xy[[t]]$X)[2]+0.01))
...
i="T45R"
plotfilename <- paste("E:/CAF/2022-10-20/fungi/fig4/",i,".tiff",sep = "")
df <- c()
for (j in c("May","June","July","August","September")) {
  filename <- paste("E:/CAF/2022-10-20/fungi/fig3/",i,"/links_gene_",j,".csv",sep = "")
...
tiff("E:/CAF/2022-10-20/fungi/fig5/Fig5.tiff",width=20,height=27,units = "cm",res=300)
par(mfcol=c(k,6))
par(mar = c(0, 0, 0,0),oma = c(1, 1, 1, 1),mgp=c(1,0.5,0))
...
for (j in 1:6) {
  ...
  par_dat <- paste("E:/CAF/2022-10-20/fungi/fig3/a=0.001-lop_par",j,".RData",sep = "")
  load(par_dat)
  ...
  for (i in 1:k) {
    ...
    plot(plot_data$x,plot_data$sum,col="#0dceda",type = "n",lwd=2,ylab = "",xlab = "",axes=FALSE,
         ylim = c(-0.3,1),xlim = c(range(plot_data$x)[1]-0.02,range(plot_data$x)[2]+0.08))
    ...
    if(length(dep_name)==1){
      text(x=max(as.numeric(plot_data$x))+0.02, 
           y=plot_data[nt,2:(ncol(plot_data)-3)]+0.18, 
           labels=dep_name,cex=1.3)
    }else{
      text(x=rep(max(as.numeric(plot_data$x))+0.02, length(dep_name)), 
           y=seq(min(plot_data[,2:(ncol(plot_data)-3)]),max(0.2,max(plot_data[,2:(ncol(plot_data)-3)])),length=length(dep_name)), 
           labels=dep_name,cex=1.3)
    }
